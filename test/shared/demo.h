...
static int createFont(NVGcontext *ctx, const char *name, const char *filename)
{
    QString fn = QString::fromUtf8(filename);
    QByteArray font = getFile(fn);
    if (font.isEmpty())
        return -1;
    unsigned char *fontData = (unsigned char *) malloc(font.size());
    memcpy(fontData, font.constData(), font.size());
    return nvgCreateFontMem(ctx, name, fontData, font.size(), 1);
}

static int createImage(NVGcontext *ctx, const char *filename)
{
    QString fn = QString::fromUtf8(filename);
    QImage img;
    img.load(fn);
    if (img.isNull())
        return 0;
    return nvgCreateImageRGBA(ctx, img.width(), img.height(), 0, img.constBits());
}

// upstream NanoVG demo
...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
    ...
    for (i = 0; i < 12; i++) {
        ...
        data->images[i] = createImage(vg, file);
        if (data->images[i] == 0) {
    ...
    data->fontIcons = createFont(vg, "icons", ":/fonts/entypo.ttf");
    if (data->fontIcons == -1) {
        qWarning("Could not add font icons.\n");
        return -1;
    }
    data->fontNormal = createFont(vg, "sans", ":/fonts/Roboto-Regular.ttf");
    if (data->fontNormal == -1) {
        qWarning("Could not add font italic.\n");
        return -1;
    }
    data->fontBold = createFont(vg, "sans-bold", ":/fonts/Roboto-Bold.ttf");
    if (data->fontBold == -1) {
        qWarning("Could not add font bold.\n");
        return -1;
    }
    data->fontEmoji = createFont(vg, "emoji", ":/fonts/NotoEmoji-Regular.ttf");
    if (data->fontEmoji == -1) {
        qWarning("Could not add font emoji.\n");
        return -1;
    }
